...
<html>
  <head>
    ...
    <title>My JSP 'wakeUser.jsp' starting page</title>
   ...
   <p>你好！</p>
			<h3>下列是被冻结的用户，请点击激活</h3>
				...
				<table width=100% border="1" bgcolor="#22ff00">
					...
					String sql2 = "select * from USER_INFOR_TAB where is_used=0";
					List<UserBean> list4 = ub.getAllUsers(con, sql2);
					session = request.getSession();
					session.setAttribute("IS_used",list4);
					List<UserBean> users = (List<UserBean>)session.getAttribute("IS_used");
					if(users!=null)
					{
					...
					%>
					<form action="/TEST_MVC/wakeUser_do.mis" method=post>
						<tr bordercolor=#990066>
							...
							<input type="hidden" name="Is_used" value="<%=uid%>"/>
							<td><input type=submit name="submit" value="激活"></td>
						</tr>
					</form>
					<%
					}

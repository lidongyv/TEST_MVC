...
public class FrozenUserAction {
	public void execute(HttpServletRequest request,HttpServletResponse response) 
			throws ServletException,IOException{
	    String login_id = request.getParameter("Not_used");
		int j = -1;
		PreparedStatement pst = null;
		Connection con = null;
		try{
		String sql ="update user_infor_tab set is_used =0 where login_id=?";
		...
		String sql = "select * from USER_INFOR_TAB where is_used=1";
		List<UserBean> list = ub.getAllUsers(con, sql);
		request.getSession().setAttribute("NOT_used", list);
		if(j == 1 ){
		RequestDispatcher rd = request.getRequestDispatcher("page_jsp/products/frozenUserSuccess.jsp");
		rd.forward(request, response);
		}
		else{
			RequestDispatcher rd = request.getRequestDispatcher("page_jsp/products/frozenUserFailed.jsp");
			rd.forward(request, response);
		}
